  ...
  <q-page class="bg-grey-1 q-pb-md">
    <div class="q-pa-md">
      ...
      <template v-else>
        ...
        <!-- Manager -->
        <q-card flat bordered class="custom-card q-mb-md cursor-pointer" @click="messageManager">
          <q-card-section class="row items-center">
            <q-avatar size="44px" color="teal-8" text-color="white" class="text-weight-bold">{{ lease?.managerInitials }}</q-avatar>
            <div class="q-ml-md col">
              <div class="text-subtitle2 text-weight-bold">{{ lease?.managerName }}</div>
              <div class="text-caption text-grey-6">Your manager</div>
            </div>
    ...
    <q-dialog v-model="leaveDialog">
      <q-card class="rounded-borders" style="min-width:300px; max-width:90vw">
        <q-card-section>
          ...
          <div class="text-grey-7 q-mt-xs">Your manager will be notified. Your stay stays active until they confirm your request.</div>
        </q-card-section>
...
async function loadStay() {
  ...
  try {
    ...
    const managerId = row.accommodation_manager_id ?? null
    ...
    if (managerId) {
      ...
      managerName = propBiz || full || 'Manager'
    }
    ...
    lease.value = {
      ...
      managerName,
      managerInitials: initialsOf(managerName),
      managerId,
    }
  } catch (e) {
...
async function requestLeave() {
  ...
  try {
    ...
    if (lease.value.managerId) {
      try {
        await createNotification(lease.value.managerId, 'Leave requested', `${lease.value.propertyName || 'Your tenant'} asked to leave (${lease.value.roomUnit}). Review it from Tenants.`, 'leave', '/manager/tenants')
      } catch { /* non-critical */ }
    }
...
function messageManager() {
  if (lease.value?.managerId) {
    void router.push({ path: '/student/messages', query: { manager: lease.value.managerId } })
  }
}
