<template>
  <q-page class="messages-page">
    <main v-if="!activeConversation" class="conversation-list" aria-labelledby="messages-title">
      ...
      <EmptyState v-else icon="lucide:message-circle" :title="searchText ? 'No conversations match your search.' : 'No conversations yet.'" :message="searchText ? '' : 'Messages from managers and managers will appear here.'" />
    </main>
    ...
    <q-dialog v-model="newConvoDialog" position="bottom">
      <q-card class="new-conversation-dialog">
        <q-card-section class="dialog-heading">
          <div><p class="eyebrow">New message</p><h2>Choose a manager</h2></div>
        ...
        <q-card-section v-if="newConvoLoading" class="list-state" role="status"><q-spinner color="primary" size="24px" /><span>Loading managers...</span></q-card-section>
        <q-card-section v-else-if="managers.length === 0" class="list-state"><span>No managers are available to message.</span></q-card-section>
        <q-card-section v-else class="dialog-list">
          <button v-for="manager in managers" :key="manager.id" type="button" class="thread-row" @click="startChatWith(manager.id, manager.name)">
            <q-avatar class="thread-avatar" size="40px">{{ initialsOf(manager.name) }}</q-avatar>
            <span class="thread-copy"><span class="thread-name">{{ manager.name }}</span><span class="thread-subtitle">Manager</span></span>
...
const filteredConversations = computed(() => {
  ...
  else if (activeFilter.value === 'landlords') list = list.filter((chat) => chat.role === 'Manager')
...
function roleLabel(role: string | null | undefined): string {
  return role === 'accommodation_manager' || role === 'manager' ? 'Manager' : role === 'student' ? 'Student' : 'Participant'
}
...
async function startNewConversation() {
  ...
  try {
    ...
    if (queryError) throw queryError
    managers.value = ((data ?? []) as any[]).map((u) => ({ id: u.id, name: u.full_name ?? 'Manager' }))
  } catch {
    managers.value = []
  } finally {
...
async function openManagerFromQuery() {
  const managerId = route.query.manager as string | undefined
  if (!managerId || !currentUserId.value) return
  const roomId = route.query.room as string | undefined
  if (roomId) roomContext.value = { roomId, managerId: managerId }
  // If a conversation with this manager already exists on this page, open it.
  const existing = conversations.value.find((c) => c.otherUserId === managerId)
  if (existing) {
  ...
  try {
    const cid = await ensureConversation(managerId)
    if (cid) {
      await loadConversations()
      await openConversation(cid)
    }
...
async function sendRoomApply() {
  ...
  try {
    ...
    try {
      await createNotification(managerId, 'New room application', 'A student applied to one of your rooms. Review it from Tenants.', 'application', '/manager/tenants')
