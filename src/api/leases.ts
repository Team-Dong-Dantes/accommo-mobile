 ...
 * application therefore had a landlord/landlady to raise a concern against on one screen
...
/** Statuses that need the landlord/landlady to do something. */
...
export async function fetchCurrentLease(
  ...
  const { data, error } = await supabase
    .from('leases')
    .select('id, status, landlord_id, rooms(room_number, accommodations(name))')
...
/** Every lease between one student and one landlord/landlady, newest first. */
...
/** How many leases are waiting on this landlord/landlady — the nav badge count. */
