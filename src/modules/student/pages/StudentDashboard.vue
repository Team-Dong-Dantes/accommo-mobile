<template>
  <q-layout view="hHh lpR fFf" class="bg-grey-1">
          ...
          <template v-else>
            ...
            <q-card flat class="q-mx-md q-mb-md custom-card" style="background: #FFF3E0;">
              <q-card-section class="q-py-sm row items-center">
                ...
                <q-btn unelevated color="orange-9" label="Pay" class="rounded-borders text-weight-bold" size="sm" no-caps @click="navigateMenu('/payments')" />
              </q-card-section>
            </q-card>
    ...
    <div v-if="fabExpanded" class="fab-overlay transition-fade" @click="fabExpanded = false">
      <div class="absolute-bottom-right column items-end q-pa-md" style="bottom: 70px;">
        <q-btn rounded color="white" text-color="dark" label="Support" class="q-mb-sm shadow-2 menu-btn" no-caps @click.stop="navigateMenu('/support')">
          <q-icon left name="help_outline" color="teal-8" />
        </q-btn>
        <q-btn rounded color="white" text-color="dark" label="Concerns" class="q-mb-sm shadow-2 menu-btn" no-caps @click.stop="navigateMenu('/concerns')">
          <q-icon left name="chat_bubble_outline" color="purple-6" />
        </q-btn>
        <q-btn rounded color="white" text-color="dark" label="Pay" class="q-mb-md shadow-2 menu-btn" no-caps @click.stop="navigateMenu('/payments')">
          <q-icon left name="credit_card" color="teal-8" />
        </q-btn>
        <q-btn round color="dark" icon="close" size="lg" class="shadow-4 q-mr-xs" @click.stop="fabExpanded = false" />
      </div>
    </div>

    <q-page-sticky v-if="!fabExpanded" position="bottom-right" :offset="[18, 70]">
      <q-btn fab icon="add" color="teal-8" class="shadow-4" @click="fabExpanded = true" />
    </q-page-sticky>
  </q-layout>
</template>
...
const quickActions = [
  { label: 'Report Issue', icon: 'report_problem', color: 'amber-8', handler: () => navigateMenu('/concerns') },
  { label: 'File Complaint', icon: 'gavel', color: 'negative', handler: () => navigateMenu('/concerns') },
  { label: 'View History', icon: 'history', color: 'teal-7', handler: () => navigateMenu('/payments') },
  { label: 'Review Stay', icon: 'star', color: 'yellow-8', handler: () => navigateMenu('/support') },
];
...
function navigateMenu(route: string) {
  fabExpanded.value = false;
  void router.push(`/student${route}`);
}

function goToStay() { navigateMenu('/support'); }
